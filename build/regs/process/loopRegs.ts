import { objectOpt } from "@ucbuilder:/build/common";
import { patternMatcher } from "@ucbuilder:/build/regs/patternMatcher";
namespace ucbuilder.build.regs.precess {
    export class loopRegs {
        loopPattern: patternMatcher;

        constructor() {
            this.loopPattern = new patternMatcher(
                /`[ \n\r]*{(loop\w*)=([\.\w]+?)}[ \n\r]*`/,
                /([^]*?)/,
                /`[ \n\r]*{\/\1}[ \n\r]*`/g
            );
        }

        parse(content: string, nodes: {}, loopEachItemCallback: (obj: {}, content: string, nameSpace: string, loopCode: string) => string = (obj = {}, content, nameSpace, loopCode) => { return content; }): string {
            let _this = this;
            return content.replace(this.loopPattern.pattern, function (
                match: string,
                loopCode: string,
                valtoFind: string,
                subcontent: string,
                offset: number,
                input_string: string
            ) {
                let result = "";
                let arlist: [{}] = objectOpt.getValByNameSpace(nodes, valtoFind);
                arlist.forEach(arVal => {
                    result += loopEachItemCallback(arVal, subcontent, valtoFind, loopCode);
                });
                return result;
            });
        }

        parseDirect(
            node: {},
            loopCode: string,
            valtoFind: string,
            subcontent: string,
            loopEachItemCallback: (obj: {}, content: string, nameSpace: string, loopCode: string) => string = (obj = {}, content, nameSpace, loopCode) => { return content; }
        ): string {
            let result = "";
            let arlist: [{}] = objectOpt.getValByNameSpace(node, valtoFind);
            arlist.forEach(arVal => {
                result += loopEachItemCallback(arVal, subcontent, valtoFind, loopCode);
            });
            return result;
        }
    }
}