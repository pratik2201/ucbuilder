...
export class CommonEvent<F extends (...arg: any) => void> {
    ...
    private static STAMP_KEY = `CommonEvent_${GetUniqueId()}`;
    private static STAMP_NO = 0;
     ...
     * @returns 
     */
    on(callback: F, uc?: Usercontrol, makeSureOnlyOneCallbackShouldUse = false, stamp: string = CommonEvent.nStamp): string {
